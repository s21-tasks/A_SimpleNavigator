...
void RandomGraph::Func() {
    Style::InputRequest({"directed (1) or undirected (0)", "size of graph (int)",
                        "probability of zero (int %)", "max value in graph (int)"});
    bool directed = Read<bool>();
    int size = Read<int>();
    int pz = Read<int>();
    int max = Read<int>();
    if (Allowed()) {
         ((Interface*)ci_)->GetGraph().CreateRandom(directed, size, (float)pz / 100.0, max);
    }
}
...
void TwoVertices::Func() {
    ...
    int end = Read<int>();
    if (Allowed()) {
        SStr::Print(GraphAlgorithms::getShortestPathBetweenVertices(((Interface*)ci_)->GetGraph(), start, end));
    }
}
...
Interface::Interface() {
    ...
    home_menu->Connect(3, AddInput(new OneValueInput<std::string>(this, home_menu,
        [&] (std::string &path) { graph_.GraphToFile(path); },
        "the path to the file")));

    home_menu->Connect(4, AddInput(new OneValueInput<int>(this, home_menu,
        [&] (int &start) { SStr::Print(GraphAlgorithms::breadthFirstSearch(graph_, start)); },
        "start vertex")));

    home_menu->Connect(5, AddInput(new OneValueInput<int>(this, home_menu,
    ...
    auto salesman_manu = AddMenu({
            "Ant colony", "Genetic", "Branch and Bounds"});
    ...
    home_menu->Connect(10, AddInput(new OneValueInput<int>(this, home_menu, [&] (int &N) {
