 ...
 *
 * The Graph class provides functionality to create, manipulate, and export graphs.
 * It supports both directed and undirected graphs, and provides methods to load graphs
 * from files, export graphs to DOT format, and perform various operations on graphs.
 */
class Graph {
private:
    bool directed_;
...
public:
     ...
     * @return A reference to the element at the specified position in the adjacency matrix.
     */
    int& operator()(int row, int col);

    /**
     * @brief Access the element at the specified position in the adjacency matrix (const version).
     * @param row The row index of the element.
     * @param col The column index of the element.
     * @return A const reference to the element at the specified position in the adjacency matrix.
     */
    ...
    @return A boolean value indicating whether the graph is directed (true) or undirected (false).
    */
    bool Directed() const;

    /**
    @brief Load a graph from a file.
    * This function loads the graph properties and adjacency matrix from a file and updates the graph accordingly.
    ...
    * This function exports the graph to a DOT file format, which is commonly used for graph visualization.
    @param filename The path to the file where the DOT representation of the graph will be saved.
    */
    void ExportGraphToDot(const std::string& filename);

    /**
    @brief Export the graph to a file in txt format.
    * This function exports the graph properties and adjacency matrix to a file in a specific format.
    ...
    * If the specified file has an extension ".dot" this function is is equivalent to ExportGraphToFile(const std::string& filename).
    * Else this function is is equivalent to ExportGraphToDot(const std::string& filename).
    @param filename The path to the file where the graph data will be saved.
    */
